...
#if defined(NDEBUG)
# error "GainX cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "GainX Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s GAINX additional fees, %d delta bytes\n",
...
{
    if (nHeight == 1){
        CAmount nSubsidy = 2000000000 * COIN;
        return nSubsidy;
    }
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // GainX: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
